...
shrimp production. Three screens: a transit-style **Line Map**, a satellite **Live Map**, and a
delivery **Retro** analysis. Vanilla HTML/JS + `<canvas>`, no build step to *run* it, works offline.
...
2. Regenerate the encrypted page. The password is passed via an env var and is **never** committed:
   ```
   CC_PW='<deployment-password>' node build-encrypt.js source.html index.html
   ```
3. Commit the regenerated `index.html` here and push — GitHub Pages redeploys automatically.

To change the password, just rebuild with a different `CC_PW` and push.
